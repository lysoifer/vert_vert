---
titlG: "Vertical niches 2.0 (birds)"
author: "Brunno F Oliveira^1^; Brett R Scheffers^2^;"
datG: "May 9, 2017 **1** Department of Wildlife Ecology and Conservation, University of Florida/IFAS, Gainesville, FL 32611, USA. **Corresponding author:** (brunno.oliveira@me.com)"
output:
...
if(answer=="no"){
  load(paste(savedatafolder,"birds",'_data.RData',sep=""))
}
if(answer=="yes"){
  occr <- read.csv(paste(savedatafolder,"birds",'_occr_equal_area_30land.csv',sep=""))
...
### OPEN LARGE DATASET
trait <- fread(paste(sep="",savedatafolder,"Elton_birds.txt"))

names(trait)

# Get traits of interest
trait <- data.frame(trait[,-c(1:7,9,21:23,32:35,37:40)]) 
trait <- na.omit(trait)
...
_Legend:_  
ForStrat-watbelowsurf: Foraging below the water surfaces  
ForStrat-wataroundsur: Foraging on or just (<5 inches) below water surface  
ForStrat-ground: Foraging on ground  
ForStrat-understory: Foraging below 2m in understory in forest, forest edges, bushes or shrubs  
ForStrat-midhigh: Foraging in mid to high levels in trees or high bushes (2m upward), but below canopy  
ForStrat-canopy: Foraging in or just above (from) tree canopy  
ForStrat-aerial: Foraging well above vegetation or any structures  
PelagicSpecialist: Foraging predominantly pelagic    

## How many species in each trait?
```{r count_traits, eval=F}
#habitat
apply(trait[,c(13:20)], 2, FUN = function(x) length(which(x>0)))
#food
apply(trait[,c(2:11)], 2, FUN = function(x) length(which(x>0)))
```

## Remove some species
```{r traits, eval=F}
# RemovG: ForStrat-watbelowsurf, ForStrat-wataroundsur, ForStrat-aerial, PelagicSpecialist.
trait <- trait[-which(trait$ForStrat.watbelowsurf>0),]
trait <- trait[-which(trait$ForStrat.wataroundsur>0),]
trait <- trait[-which(trait$ForStrat.aerial>0),]
#trait <- trait[-which(trait$PelagicSpecialist==1),] # don not run this. All the PelagicSpecialist have been removed in the lines above. Thus, if you run this it will return an error!

trait <- trait[,-c(13,14,19,20)]

trait_names <- data.frame(gsub(" ","_",trait$Scientific))
```
...
# vert.temp <- trait[,c(12:15)]
# names(vert.temp)
# 
# vert.temp[,2] <- vert.temp[,2]*2 # understory
# vert.temp[,3] <- vert.temp[,3]*3 # midhigh
# vert.temp[,4] <- vert.temp[,4]*4 # canopy
# 
# vert <- rowSums(vert.temp)

vert.temp <- trait[,c(12:15)]
names(vert.temp)

vert.temp$ForStrat.ground[which(vert.temp$ForStrat.ground>0)] <- 1
vert.temp$ForStrat.understory[which(vert.temp$ForStrat.understory>0)] <- 2
vert.temp$ForStrat.midhigh[which(vert.temp$ForStrat.midhigh>0)] <- 3
vert.temp$ForStrat.canopy[which(vert.temp$ForStrat.canopy>0)] <- 4
vert.temp[vert.temp == 0] <- NA

vert <- unlist(apply(vert.temp, 1, function(x) mean(x, na.rm = T)))
...
The marjority of species got lower verticality values, meaning that they forrage close to the ground.  

## Vertical plasticity  
We also calculated the vertical flexibility of species as the number of vertical layers used by species.
```{r eval=F}
vert.plast <- apply(trait[,c(12:15)], 1, function(x) length(which(x>0)))

trait <- data.frame(trait,vert.plast)
```

## Food plasticity  
We also calculated the vertical food of species as the number of food itens used by species.
```{r eval=F}
food.plast <- apply(trait[,c(1:10)], 1, function(x) length(which(x>0)))
...
veg.r <- raster(paste(sep="",externHD,'#Environment/NASA veg height/asc/veg-height.asc'))
veg.r <- resample(veg.r, bio, method= "bilinear")
veg.r <- projectRaster(veg.r, crs = crs(mapa, asText = T))
...
Reanames <- readOGR("G:/GIS/CMEC regions & realms/newRealms.shp")
...
#2) Extract values
Realm <- readOGR("G:/GIS/CMEC regions & realms/newRealms.shp")
...
Realm2 <- readOGR("G:/GIS/CMEC regions & realms/newRealms.shp")
...
Realm[which(is.na(Realm))] <- Realm2[which(is.na(Realm))]
...
write.csv(sp.vars, paste(savedatafolder,"birds","_spvars.csv",sep=""))
...
body.cell <- apply(occr, 1, function(x) mean(trait$BodyMass.Value[which(x==1)]))
# Mean community range size
range.size.cell <- apply(occr, 1, function(x) mean(range.size[which(x==1)]))
# Mean community Verticality
vert.cell <- apply(occr, 1, function(x) mean(vert[which(x==1)]))
# Mean community vertical plasticity
vert.plast.cell <- apply(occr, 1, function(x) mean(vert.plast[which(x==1)]))
...
for (i in 1:NCell) {
  ...
  if (is.na(Realm[i])) {
    ses.vert[i] <- ses.vert.plast[i] <- ses.food.plast[i] <- NA
  }
  else {
    Realm.cells <-
    ...
    null.vert <- replicate(100, mean(sample(vert[pool], Rich[i], replace = F)))
    ses.vert[i] <- (vert.cell[i] - mean(null.vert)) / sd(null.vert)
    
    null.vert.plast <- replicate(100, mean(sample(vert.plast[pool], Rich[i], replace = F)))
    ses.vert.plast[i] <- (vert.plast.cell[i] - mean(null.vert.plast)) / sd(null.vert.plast)
...
write.csv(com.vars, paste(savedatafolder,"birds","_comvars.csv",sep=""))
...
write.csv(Realm.vars, paste(savedatafolder,"birds_Realmvars.csv",sep = ""))
...
write.csv(biorealm.vars, paste(savedatafolder,"birds_biorealmvars.csv",sep = ""))
```
...
# SAVE
```{r echo=F, eval=FALSE}
save.image(paste(savedatafolder,"birds",'_data.RData',sep=""))
```